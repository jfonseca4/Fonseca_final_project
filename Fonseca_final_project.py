...
def state_edges(election_result_rows):
    ...
    StateEdges = ()
    for row in election_result_rows: #for each row in the pollsters
        State = row("state") #identifying the state
        edge = row_to_edge(row) #identifying the row
        StateEdges[state] = edge #identofying the edges
        return StateEdges #returns the state, row, and edge
...
date1 = "pollster" , "state"  #the first date
date2 = "pollster" , "state" #the second date

if date1 == date2: #if the dates are the same
    print("none")
elif date1 < date2:  #if the first date is earleir than the second
    print(pollster , state) #prints the pollster and state
elif date1 > date2: #if the first date is later than the second
    print(pollster , state) #prints out that pollster and state
...
def pollster_predictions(poll_rows):
    ...
    for pollster in pollDataRows:

        return pollster_predictions
...
def average_error(state_edges_predicted, state_edges_actual):
    ...
    sum = state_edges_predicted + state_edges_actual #getting the sum
    average_error = sum/2 #completes the average error of the edges

    return average_error #returns the average error
...
#i was not sure what to do for this problem
...
#i did not understand what i had to do for this problem
...
def weighted_average(items, weights):
    ...
    weighted_average = w * l / sum(w) #gets weighted average of list
    #of items



    return weighted_average #returns weighted average
...
def average_edge(pollster_edges, pollster_errors):
    ...
    sum = pollster_edges + pollster_errors #gets sum of pollsters

    average_edge = sum / 2 #gets average of the edges

    return average_edge #returns average edge
...
#since i did not fully understand the whole project,i unfortunately
